structure XMLLexer
  = struct
    ...
    datatype yystart_state = DOCTYPE | COM | LIT2 | LIT1 | INITIAL | TAG
...
(* trim m characters from the left and n characters from the right *)
  fun trim (m, ss, n) = Substring.string(Substring.triml m (Substring.trimr n ss))
    ...
    fun innerLex (yyarg as  lexErr) (yystrm_, yyss_, yysm) = let
        (* current start state *)
          val yyss = ref yyss_
...
fun yyAction38 (strm, lastMatch : yymatch) = let
      ...
      in
        yystrm := strm;
         T.TEXT(UTF8.encode(Word.fromInt(Option.valOf(Int.fromString(trim(2, yysubstr, 1))))))
      end
fun yyAction39 (strm, lastMatch : yymatch) = let
      val yysubstr = yymksubstr(strm)
      in
        yystrm := strm;
         T.TEXT(UTF8.encode(Option.valOf(Word.fromString(trim(3, yysubstr, 1)))))
      end
...
fun yyQ133 (strm, lastMatch : yymatch) = (case (yygetc(strm))
       of NONE => yyAction43(strm, yyNO_MATCH)
        | SOME(inp, strm') =>
            if inp = 0wx3E
              then yyQ134(strm', yyMATCH(strm, yyAction43, yyNO_MATCH))
       ...
       of NONE => yyAction40(strm, yyNO_MATCH)
        | SOME(inp, strm') =>
            if inp = 0wx5D
              ...
              else yyQ86(strm', yyMATCH(strm, yyAction40, yyNO_MATCH))
      (* end case *))
       ...
       of NONE => yyAction31(strm, yyNO_MATCH)
        | SOME(inp, strm') =>
            if inp = 0wxE
              ...
              then if inp <= 0wx8
                  then yyAction31(strm, yyNO_MATCH)
                  else yyQ123(strm', yyMATCH(strm, yyAction31, yyNO_MATCH))
            else if inp = 0wx20
              then yyQ123(strm', yyMATCH(strm, yyAction31, yyNO_MATCH))
       ...
       of NONE => yyAction32(strm, yyNO_MATCH)
        | SOME(inp, strm') =>
            if inp = 0wx20
              ...
              then if inp = 0wxB
                  then yyQ122(strm', yyMATCH(strm, yyAction32, yyNO_MATCH))
                else if inp < 0wxB
                  then if inp <= 0wx8
                      then yyQ122(strm', yyMATCH(strm, yyAction32, yyNO_MATCH))
                  ...
                  then yyAction32(strm, yyNO_MATCH)
                  else yyQ122(strm', yyMATCH(strm, yyAction32, yyNO_MATCH))
            else if inp = 0wx27
              then yyQ122(strm', yyMATCH(strm, yyAction32, yyNO_MATCH))
            else if inp < 0wx27
              then if inp = 0wx26
       ...
       of NONE => yyAction31(strm, yyNO_MATCH)
        | SOME(inp, strm') =>
            if inp = 0wx20
              ...
              then if inp = 0wxB
                  then yyQ124(strm', yyMATCH(strm, yyAction31, yyNO_MATCH))
                else if inp < 0wxB
                  then if inp <= 0wx8
                  ...
                  then yyQ124(strm', yyMATCH(strm, yyAction31, yyNO_MATCH))
                  else yyQ122(strm', yyMATCH(strm, yyAction31, yyNO_MATCH))
            else if inp = 0wx27
              then yyQ122(strm', yyMATCH(strm, yyAction31, yyNO_MATCH))
            else if inp < 0wx27
              then if inp = 0wx26
       ...
       of NONE => yyAction42(strm, yyNO_MATCH)
        | SOME(inp, strm') =>
            if inp = 0wx3C
              ...
              then yyQ25(strm', yyMATCH(strm, yyAction42, yyNO_MATCH))
            else if inp < 0wx5F
              then if inp = 0wx3F
                  then yyQ25(strm', yyMATCH(strm, yyAction42, yyNO_MATCH))
                else if inp < 0wx3F
...
in
  ...
  (* end case *))
	        ...
	        in
                  (skip(), (!yystartPos, yygetPos()-1), !yystrm, !yyss)
                end (* lex *)
          in
            lex()
          end (* innerLex *)
    ...
    fun lex sm (yyarg as  lexErr) (STRM (yystrm, memo), ss) = (
          case !memo
           of NONE => let
                val (tok, span, yystrm', ss') = innerLex yyarg (yystrm, ss, sm)
                val strm' = STRM (yystrm', ref NONE);
                in
                  memo := SOME (ss, tok, span, strm', ss');
                  (tok, span, (strm', ss'))
                end
	    | SOME (ss', tok, span, strm', ss'') => if ss = ss'
                then (tok, span, (strm', ss''))
	        else (
		 memo := NONE;
		 lex sm yyarg (STRM (yystrm, memo), ss))
          (* end case *))

    fun streamify input = (STRM (yystreamify' 0 input, ref NONE), INITIAL)
    fun streamifyReader readFn strm =
          (STRM (yystreamifyReader' 0 readFn strm, ref NONE), INITIAL)
    fun streamifyInstream strm =
          (STRM (yystreamifyInstream' 0 strm, ref NONE), INITIAL)
